...
<body>
    ...
    <form action="<%=basePath%>admin/customer/doUpdateCustomer" method="post">
    <!-- 传递页码 -->
    <input name="page" type="hidden" value="${requestScope.page}"/>
     <input name="id" type="hidden" value="${requestScope.customer.id}"/>
    <ul class="forminfo">
    <li><label>*账户名</label><input name="username" type="text" class="dfinput" disabled="disabled" value="${requestScope.customer.username}"/><i>${requestScope.errors["username"]}</i></li>
    ...
    <li><label>客户简介</label>
        <textarea name="intro" cols="" rows="" class="textinput">${requestScope.customer.intro}</textarea><i>${requestScope.errors["intro"]}</i>
    </li>  
