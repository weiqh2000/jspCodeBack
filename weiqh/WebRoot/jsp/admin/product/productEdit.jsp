 ...
 //用于弹出窗口，将服务器返回的数据提交，本处用于账户提交后的状态
	...
	KindEditor.ready(function(K) {  
	           editor2 = K.create('#content', {//文本编辑器
	                ...
	                uploadJson:  '<%=basePath%>/plugins/kindeditor-4.1.10/jsp/upload_json.jsp;jsessionid='+'CB6B6CDE141E69A85D40DB54974105B2',
	                //指定浏览远程图片的服务器端程序
	                fileManagerJson: '<%=basePath%>/plugins/kindeditor-4.1.10/jsp/file_manager_json.jsp;jsessionid='+'CB6B6CDE141E69A85D40DB54974105B2',
...
<body>
    ...
    <form action="<%=basePath%>admin/product/EditddProduct" method="post">
    <!-- 传递页码 -->
    ...
    <ul class="forminfo">
      <li>
        ...
        <select name="productTypeId"  class="dfinput" >
             ...
             <c:forEach var="productTypeOfEach" items="${requestScope.productType}">
                 <option value="${productTypeOfEach.id}">${productTypeOfEach.name}</option>  
        		</c:forEach> 
        </select>
        <i></i>
     </li>
    ...
    <li><label>*产品内容</label>
        <textarea id="content" name="content" class="textinput">${requestScope.product.content}</textarea>
        <i></i>
    </li>
