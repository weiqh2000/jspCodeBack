...
<body>
    ...
    <form action="<%=basePath %>admin/product/EditddProductType" method="post">
    <!-- 传递页码 -->
    <input name="Id" type="hidden" id="Id" class="dfinput" value="${productType.id}"/>
    <ul class="forminfo">
        
    <li><label>*产品分类名称</label><input name="name" type="text" class="dfinput" value="${productType.name}"><i></i></li>
    <li><label>*产品图片</label>
        <input id="url1" name="imageUrl" type="text" class="dfinput" value="${productType.imageUrl}" readonly="readonly">
        <input type="button" id="image1" class="dfinput" style="width:120px;" value="点我选择图片">	
        <a href="#" id="clearImagePath1">清除选择图片路径</a>	
        <i></i>
    </li>
    <li><label>*优先级</label><input name="orderNum" type="text" class="dfinput" value="${productType.orderNum}"><i></i></li>
    <li><label>外部链接地址</label><input name="linkUrl" type="text" class="dfinput" value="${productType.linkUrl}"><i></i></li>
    <li><label>产品描述</label>
        <textarea name="intro" cols="" rows="" class="textinput" value="${productType.intro}"></textarea><i></i>
    </li>
