 ...
 * 处理产品的编辑请求
 */
@WebServlet(urlPatterns="/admin/product/doUpdateProduct")
public class ProductDoUpdateProductServlet extends HttpServlet {
	private ProductTypeBean productTypeBean=new ProductTypeBean();
	private ProductBean productBean=new ProductBean();
	private AdminBean adminBean=new AdminBean();

	public ProductDoUpdateProductServlet() {
		super();
	}
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		if(errors.size()==0){//说明表单验证通过
			try {
				if(productBean.updateProduct(product,(Admin)session.getAttribute("admin"))){
					request.setAttribute("myMessage", "产品编辑成功");
					//初始化表单
					request.setAttribute("product", productBean.getProduct(product.getId()));
				}else{
					request.setAttribute("myMessage", "产品编辑失败");
				}
	...
	/*
	 * 针对产品编辑请求进行验证并对部分属性进行赋值,用于配合产品添加方法
	 * @param request
	 * @param errors 错误信息集合
